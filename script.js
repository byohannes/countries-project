...
function getBorderName(borderCodes) {
    ...
    borderCodes.forEach((code) => {
        ...
        nameArr.push(
            `<button type="button" class="d-flex justify-content-start mr-md-2 btn btn-outline-secondary border-buttons borderCountries">${name}</button>`,
        )
    })
    if (nameArr.length === 0) {
        return 'No Border'
